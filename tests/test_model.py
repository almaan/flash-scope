...
def model_params():
    ...
    logits = (rng.random(n_genes).astype(np.float32) - 0.5) * 4
    return R, logits, n_spots, n_types, n_genes


class TestFlashScopeModel:
    def test_init(self, model_params):
        R, logits, n_spots, n_types, n_genes = model_params
        model = FlashScopeModel(R, logits, n_spots, n_types, n_genes)
        assert model.w.shape == (n_spots, n_types + 1)
        assert model.beta.shape == (1, n_genes)
        assert model.eta.shape == (1, n_genes)

    def test_forward_output_shape(self, model_params):
        R, logits, n_spots, n_types, n_genes = model_params
        model = FlashScopeModel(R, logits, n_spots, n_types, n_genes)
    ...
    def test_r_sc_not_trainable(self, model_params):
        ...
        assert not model.o_sc.requires_grad
